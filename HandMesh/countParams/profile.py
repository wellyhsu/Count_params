import pandas as pd
from countParams.rnn_hooks import *
from countParams.vision.basic_hooks import *
from .utils import prRed, clever_format
...
def profile(
    ...
    def dfs_count(module: nn.Module, max_params, max_params_layer_name, prefix="\t") -> (int, int, int, str,):
        ...
        for n, m in module.named_children():
            ...
            if m in handler_collection and not isinstance(m, (nn.Sequential, nn.ModuleList)):
                ...
                if m_params != float(0):
                    if m_params > max_params:
                        max_params = m_params
                        max_params_layer_name = module._get_name()
                    summary_data.append({
                ...
                m_ops, m_params, max_params, max_params_layer_name, next_dict= dfs_count(m, max_params, max_params_layer_name, prefix=prefix + "\t")                
            ret_dict[n] = (m_ops, m_params, next_dict)
            total_ops += m_ops
            total_params += m_params
            
        return total_ops, total_params, max_params, max_params_layer_name, ret_dict
   
    total_ops, total_params, max_params, max_params_layer_name, ret_dict = dfs_count(model, max_params, max_params_layer_name)
    
     # 將結果保存為 Excel 檔案
    df = pd.DataFrame(summary_data)
    df.to_excel('model_params.xlsx', index=False)
    ...
    return total_ops, total_params, max_params, max_params_layer_name
